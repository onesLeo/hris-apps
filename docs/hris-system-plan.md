...
- Requisition priority is now a persisted ATS field on `job_requisitions`, so the create/update flow and UI reflect real-world urgency instead of only a visual badge.
- The recruitment screen now includes a requisition detail view with approval workflow timeline and linked applications, and requisition rows persist the active workflow instance id plus readable department, location, and hiring manager names.
