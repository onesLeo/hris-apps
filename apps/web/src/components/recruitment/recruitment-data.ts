...
export type CreateRequisitionInput = Omit<OpenRequisition, 'accent'>;
...
export function addRecruitmentRequisition(
  ...
  return [
    {
      ...input,
      accent: input.priority === 'High' ? '#e8317a' : input.priority === 'Medium' ? '#8b5cf6' : '#0ea5e9',
    },
    ...requisitions,
  ];
}
...
export function getRecruitmentRequisitionKey(requisition: OpenRequisition): RecruitmentRequisitionKey {
  return [requisition.title, requisition.department, requisition.location, requisition.recruiter].join('|');
}
